...
Compare ranking and citation behavior across major AI platforms so optimization
decisions can be prioritized by channel.

## Load When

- You need platform-specific optimization priorities.
- You are deciding where to invest first across AI channels.
- You need crawler/index prerequisites for a specific platform.

## Quick Checklist

- [ ] Verify your content is indexed in each target platform backend.
- [ ] Verify target platform crawlers are not blocked.
- [ ] Apply the top ranking factors for the chosen platform.
- [ ] Re-check citation performance after implementation.
